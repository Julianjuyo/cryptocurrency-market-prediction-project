...
class Price(BaseModel):
    ...
    open_price: float = Field(..., example=20000.2134, gt=0)
    close_price: float = Field(..., example=22000.214, gt=0)
    low_price: float = Field(..., example=19500.2, gt=0)
    high_price: float = Field(..., example=22500.3, gt=0)
    volume: float = Field(..., example=72.3, gt=0)
